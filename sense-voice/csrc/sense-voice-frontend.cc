...
static void fbank_feature_batch_worker_thread(int ith,
                                        const std::vector<double> &hamming,
  ...
  std::vector<double> window;
  const int padded_window_size = round_to_nearest_power_two(frame_size);
  fprintf(stderr, "4444444444444444 %d\n", padded_window_size);
  window.resize(padded_window_size);
  ...
  for (; i < n_fft; i += n_threads) {
    ...
    for(int batch = 0; batch < samples.size(); batch++) {
      std::copy(samples[batch].begin() + offset, samples[batch].begin() + offset + frame_size,
                window.begin());
      ...
      // log-Mel filter bank energies aka: "fbank"
      {
        ...
        for (int j = 0; j < n_mel; j++) {
          ...
          mel.data[batch * n_fft * n_mel + i * n_mel + j] = static_cast<float>(sum);
        }
      }
    }
  }
...
bool fbank_lfr_cmvn_feature_batch(const std::vector<std::vector<double>> &samples,
  ...
  feats.n_len = 1 + ((n_samples - frame_size * n_frames_per_ms) /
                   (frame_step * n_frames_per_ms));
  fprintf(stderr, "2222222222222222 %d %d %d %d %d\n", samples.size(), samples[0].size(), n_samples, feats.n_mel, feats.n_len);
  feats.data.resize(feats.n_mel * feats.n_len);
...

  {
    if (n_threads > 1) {
        ThreadPool pool(n_threads);
        for (int iw = 0; iw < n_threads - 1; ++iw) {
            pool.enqueue(fbank_feature_batch_worker_thread, iw + 1, std::cref(hamming),
                         samples, n_samples, frame_size * n_frames_per_ms,
                         frame_step * n_frames_per_ms, n_threads, std::ref(feats));
        }
    }
  ...
  fprintf(stderr, "33333333333333333\n");
    // main thread
    fbank_feature_batch_worker_thread(0, hamming, samples, n_samples,
                                frame_size * n_frames_per_ms,
                                frame_step * n_frames_per_ms, n_threads, feats);
  }
  if (debug) {
  ...
  for (int i = 0; i < T_lrf; i++) {
    ...
    if (i == 0) {
      // left padding
      for (int j = 0; j < left_pad; j++) {
        p.insert(p.end(), feats.data.begin() + batch * feats.n_len * feats.n_mel, 
                feats.data.begin() + batch * feats.n_len * feats.n_mel + feats.n_mel);
      }
      p.insert(p.end(), feats.data.begin() + batch * feats.n_len * feats.n_mel, 
              feats.data.begin() + batch * feats.n_len * feats.n_mel + left_pad_offset);
      out_feats.push_back(p);
      p.clear();
    } else {
      if (lfr_m <= T - i * lfr_n) {
        p.insert(p.end(), feats.data.begin() + batch * feats.n_len * feats.n_mel + (i * lfr_n - left_pad) * feats.n_mel,
                    feats.data.begin() + batch * feats.n_len * feats.n_mel + (i * lfr_n - left_pad + lfr_m) * feats.n_mel);
        out_feats.push_back(p);
        p.clear();
      } else {
        // Fill to lfr_m frames at last window if less than lfr_m frames  (copy
        ...
        for (int j = 0; j < (feats.n_len - i * lfr_n); j++) {
          p.insert(p.end(),
                      feats.data.begin() + batch * feats.n_len * feats.n_mel + (i * lfr_n - left_pad) * feats.n_mel,
                      feats.data.begin() + (batch + 1) * feats.n_len * feats.n_mel);
        }
        for (int j = 0; j < num_padding; j++) {
          p.insert(p.end(), feats.data.begin() + (batch + 1) * feats.n_len * feats.n_mel - feats.n_mel, 
                  feats.data.begin() + (batch + 1) * feats.n_len * feats.n_mel);
        }
        out_feats.push_back(p);
        p.clear();
  ...
  feats.data.resize(samples.size() * T_lrf * feats.lfr_m * feats.n_mel);
  // apply cvmn
  for (int batch = 0; batch < samples.size(); batch++)
  for (int i = 0; i < T_lrf; i++) {
    for (int j = 0; j < feats.lfr_m * feats.n_mel; j++) {
        feats.data[batch * T_lrf * feats.lfr_m * feats.n_mel + i * feats.lfr_m * feats.n_mel + j] = 
            (out_feats[batch * T_lrf + i][j] + cmvn.cmvn_means[j]) * cmvn.cmvn_vars[j];
    }
  }
  return true;
}
