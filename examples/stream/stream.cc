...
void sense_voice_stream_usage(int /*argc*/, char ** argv, const sense_voice_stream_params & params) {
    ...
    fprintf(stderr, "            --step N        [%-7d] [Device] audio step size in milliseconds\n",                 params.step_ms);
    fprintf(stderr, "            --length N      [%-7d] [Device] audio length in milliseconds\n",                    params.length_ms);
    fprintf(stderr, "            --keep N        [%-7d] [Device] audio to keep from previous step in ms\n",          params.keep_ms);
    fprintf(stderr, "  -c ID,    --capture ID    [%-7d] [Device] capture device ID\n",                               params.capture_id);
    // fprintf(stderr, "  -mt N,    --max-tokens N  [%-7d] [Device]maximum number of tokens per audio chunk\n",      params.max_tokens);
    // fprintf(stderr, "  -ac N,    --audio-ctx N   [%-7d] audio context size (0 - all)\n",                          params.audio_ctx);
    fprintf(stderr, "  -vth N,   --vad-thold N   [%-7.2f] [VAD] voice activity detection threshold\n",               params.vad_thold);
    fprintf(stderr, "  -fth N,   --freq-thold N  [%-7.2f] [VAD]high-pass frequency cutoff\n",                        params.freq_thold);
...
int main(int argc, char** argv)
{
    ...
    const int n_samples_step = params.step_ms * 1e-3 * SENSE_VOICE_SAMPLE_RATE;
    const int n_samples_len = params.length_ms * 1e-3 * SENSE_VOICE_SAMPLE_RATE;
    const int n_samples_keep = params.keep_ms * 1e-3 * SENSE_VOICE_SAMPLE_RATE;
    const int n_sample_30s = 30000.0 * 1e-3 * SENSE_VOICE_SAMPLE_RATE;
    ...
    std::vector<double> pcmf32_double;
...

    {
        ...
        fprintf(stderr, "%s: processing %d samples (step = %.1f sec / len = %.1f sec / keep = %.1f sec), %d threads, lang = %s, task = %s, timestamps = %d ...\n",
                __func__,
                n_samples_step,
                float(n_samples_step)/SENSE_VOICE_SAMPLE_RATE,
                float(n_samples_len )/SENSE_VOICE_SAMPLE_RATE,
                float(n_samples_keep)/SENSE_VOICE_SAMPLE_RATE,
                params.n_threads,
                params.language.c_str(),
    ...
    while (is_running) {
        ...
        // run the inference
        {
...

            {
                ...
                if (sense_voice_full_parallel(ctx, wparams, pcmf32_double, pcmf32_double.size(), params.n_processors) != 0) {
            ...
            if (!use_vad && (n_iter % n_new_line) == 0) {
                ...
                if (!params.no_context) {
                    ...
                    const int n_segments = ctx->state->result_all.size();
                    for (int i = 0; i < n_segments; ++i) {
                        const int token_count = ctx->state->result_all[i].tokens.size();
                        for (int j = 0; j < token_count; ++j) {
                            prompt_tokens.push_back(ctx->state->result_all[i].tokens[j].id);
                        }
                    }
                }
            }
